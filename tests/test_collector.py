...
SCRIPT = Path(__file__).resolve().parents[1] / "collector.py"
SPEC = importlib.util.spec_from_file_location("amazon_new_release_collector", SCRIPT)
...
class CollectorTests(unittest.TestCase):
    def test_detects_access_control_pages(self) -> None:
        self.assertEqual(MODULE.detect_access_blocker("Robot Check"), "robot check")
        self.assertEqual(
            MODULE.detect_access_blocker("ordinary page", "https://www.amazon.com/ap/signin", "Amazon Sign-In"),
            "Amazon login page",
        )
        self.assertIsNone(MODULE.detect_access_blocker("Amazon Hot New Releases"))
    ...
    def test_deduplicates_asins_and_rebuilds_rank(self) -> None:
        ...
        result = MODULE.deduplicate_items(rows, 60)
    ...
    def test_deduplicate_items_sorts_by_rank_before_deduping(self) -> None:
        ...
        result = MODULE.deduplicate_items(rows, 3)
    ...
    def test_extracts_title_selling_points(self) -> None:
        ...
        self.assertEqual(
            MODULE.infer_title_selling_points(title),
            ["Leakproof Lid", "Easy Carry Handle", "BPA Free"],
        )
        self.assertEqual(MODULE.infer_title_selling_points("Plain Product Name"), [])

    def test_parses_localized_rating(self) -> None:
        self.assertEqual(MODULE.parse_first_number("4.7 out of 5 stars"), 4.7)
        self.assertEqual(MODULE.parse_first_number("4,6 von 5 Sternen"), 4.6)
    ...
    def test_store_replaces_same_day_snapshot_and_keeps_latest_30_calendar_days(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            store = MODULE.TrendStore(path, retention_days=30)
            ...
            store.prune(MODULE.date.fromisoformat("2026-09-01"))
            self.assertEqual(store.snapshot_dates(source), ["2026-09-01"])
            with MODULE.closing(store.connect()) as connection:
