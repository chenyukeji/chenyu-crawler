...
解析保留 Amazon 页面真实排名；滚动至每页最多 50 条，并在底部回滚触发遗漏的懒加载（最多 40 步），再解析，并检查 Top 100 排名是否完整。若站点稳定的第一页只展示 #1–#49、末页展示 #51–#100，且两页都无解析丢卡，则按网站实际展示的 99 条判为完成（99/99），保留“未展示第50名”说明；不会虚构 #50。其他排名缺口仍标部分完成。
...
`env/browser.json` 的 `between_pages_seconds` 控制页面完成加载后访问下一页前额外等待多久；当前为 5 秒，范围 0–300 秒。每个类目及每一页都由独立 Chromium 进程打开；等待结束后若页面出现明确限制，则停止当前类目。

## 按页恢复（本轮内）

正常翻页读取页面实际提供的下一页链接，在独立 Chromium 进程中直接打开该链接并检查返回正文。不会拼接或推算分页 URL，也不会回退到同一浏览器进程内点击翻页。该方式已在美国站完整采集实测通过，但不保证站点今后不会再次限制访问。
