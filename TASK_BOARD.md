...
建立“采集 → SQLite → 分析 → Service → MCP → 选品 Skill”的可维护结构，并以 `run_daily.py` 作为唯一每日采集入口。
...
- [x] T04 使用 `data/new_releases.db` 保存新品榜历史数据。
- [x] T05 建立 `database/schema.py`，定义 `observations`、`product_seen` 和查询索引。
- [x] T06 建立 `database/repository.py`，负责写入、10 天详细历史清理、90 天 ASIN 身份清理和只读连接。
...
- [x] T13 建立 `run_daily.py` 作为每日采集唯一入口。
- [x] T14 删除多余入口，采集命令统一通过 `run_daily.py` 执行。
- [x] T15 更新配置：详细快照 10 天，ASIN 身份 90 天。
- [x] T16 新增分析层自动测试。
- [x] T17 数据库按当前 schema 初始化并通过读写验证。
- [x] T18 验证 NEW / REPEAT / RISING / CLUSTER / CANDIDATES 查询均能返回结果。
...
- [x] W01 创建 FastAPI 管理后台基础服务。
- [x] W02 增加类目配置数据库和 API。
- [x] W03 增加类目管理页面。
- [x] W04 增加采集任务管理。
- [x] W05 增加任务调度和运行记录。
...
- 增加 SQLite `categories` 表。
- 支持 marketplace、node_id、父级类目、层级和启用状态。
- 增加类目 API：查询、创建、停用。

- 增加测试：API 路由和 categories 表结构验证。
...
- 增加 SQLite `crawl_runs` 运行记录表。
- 增加运行记录查询 API `/api/runs`。
- 增加 scheduler 基础运行记录模块。

W04 实现内容：

- 增加 SQLite `crawl_tasks` 表。
- 增加任务 API：查询、创建、停用。
- 增加任务管理页面 `/tasks`。
- 支持配置站点、类目、运行时间字段。
